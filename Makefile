# Calligraphy -- distributed job processing platform.
#
...
# CALLIGRAPHY_TEST_* env skips itself, so `go test ./...` is always safe to run.
TEST_DATABASE_URL ?= postgres://postgres:postgres@127.0.0.1:5432/calligraphy_test?sslmode=disable
...
test-integration:
	CALLIGRAPHY_TEST_DATABASE_URL="$(TEST_DATABASE_URL)" \
	CALLIGRAPHY_TEST_REDIS_ADDR="$(TEST_REDIS_ADDR)" \
...
.PHONY: proto
proto:
	protoc --go_out=. --go_opt=module=github.com/jguapp/calligraphy \
	       --go-grpc_out=. --go-grpc_opt=module=github.com/jguapp/calligraphy \
	       proto/control.proto
