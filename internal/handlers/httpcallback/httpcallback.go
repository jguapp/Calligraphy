...
// the handler that turns Calligraphy into generic infrastructure.
//
...
// its TTS pipeline); Calligraphy supplies what that application's own comments
...
type payload struct {
	...
	// Event names what this delivery is, carried in X-Calligraphy-Event so one
...
func New(secret string, allowedHosts []string) handler.Registration {
	h := &Handler{
		...
		Client: &http.Client{
			...
			// have Calligraphy vouch for a body to a host nobody registered. A
...
func (h *Handler) Handle(ctx context.Context, j *job.Job) (json.RawMessage, error) {
	...
	req.Header.Set("User-Agent", "calligraphy/1.0")
	if p.Event != "" {
		req.Header.Set("X-Calligraphy-Event", p.Event)
	}
	req.Header.Set("X-Calligraphy-Job-Id", j.ID)
	req.Header.Set("X-Calligraphy-Attempt", strconv.Itoa(j.AttemptCount))
	if h.Secret != "" {
		ts := time.Now().Unix()
		req.Header.Set("X-Calligraphy-Timestamp", strconv.FormatInt(ts, 10))
		req.Header.Set("X-Calligraphy-Signature", "v1="+Sign(h.Secret, ts, p.Body))
	}
