// Package metrics owns every Prometheus series Forge exposes. One place,
...
func New() *Metrics {
	...
	m := &Metrics{
		...
		JobsCancelled: prometheus.NewCounterVec(prometheus.CounterOpts{
			Name: "forge_jobs_cancelled_total",
		...
		WritesFenced: prometheus.NewCounter(prometheus.CounterOpts{
			Name: "forge_writes_fenced_total",
...
func (m *Metrics) RegisterWorkerGauges(workerID string, p PoolStats) {
	...
	m.reg.MustRegister(
		prometheus.NewGaugeFunc(prometheus.GaugeOpts{
			Name: "forge_worker_active_jobs", Help: "Jobs executing right now.", ConstLabels: labels,
		}, func() float64 { return float64(p.Active()) }),
		prometheus.NewGaugeFunc(prometheus.GaugeOpts{
			Name: "forge_worker_target_concurrency", Help: "Current concurrency target.", ConstLabels: labels,
		}, func() float64 { return float64(p.Target()) }),
		prometheus.NewGaugeFunc(prometheus.GaugeOpts{
			Name: "forge_worker_utilization", Help: "active / target, 0..1.", ConstLabels: labels,
		}, func() float64 {
			t := p.Target()
			if t == 0 {
				return 0
		...
		prometheus.NewCounterFunc(prometheus.CounterOpts{
			Name: "forge_worker_processed_total", Help: "Deliveries finished by this worker.", ConstLabels: labels,
...
func NewDepthCollector(q *queue.Queue) *DepthCollector {
	...
	return &DepthCollector{
		Q:            q,
		descReady:    prometheus.NewDesc("forge_queue_depth", "Entries ready to claim.", ql, prometheus.Labels{"state": "ready"}),
		descDelayed:  prometheus.NewDesc("forge_queue_delayed_depth", "Envelopes waiting on backoff/schedule.", ql, nil),
		descInflight: prometheus.NewDesc("forge_queue_inflight", "Claimed, unacked entries.", []string{"queue"}, nil),
		descDLQ:      prometheus.NewDesc("forge_queue_dlq_depth", "Entries in the DLQ log stream.", []string{"queue"}, nil),
	}
}
