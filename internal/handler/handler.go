// Package handler defines the contract between Forge's engine and the
...
//     worker slot hostage;
//   - be idempotent where they have external side effects: Forge delivers
//     at-least-once, so a handler may run more than once for the same job
//     (see the README's idempotency section for what Forge does and does
//     not guarantee).
//
