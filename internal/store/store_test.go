...
// gated: with no CALLIGRAPHY_TEST_DATABASE_URL set the whole file skips, so a
...
func newTestStore(t *testing.T) *Store {
	...
	if dsn == "" {
		t.Skip("CALLIGRAPHY_TEST_DATABASE_URL not set; skipping store integration tests")
	}
