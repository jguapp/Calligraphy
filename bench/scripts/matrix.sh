...
DB="${CALLIGRAPHY_DATABASE_URL:-postgres://postgres:postgres@127.0.0.1:5432/calligraphy?sslmode=disable}"
TOKEN="cg_bench_token"
API=http://127.0.0.1:8080
LOGDIR="${BENCH_LOGDIR:-/tmp/calligraphy-bench-logs}"
mkdir -p "$LOGDIR"
...
start_api() {
  CALLIGRAPHY_DATABASE_URL="$DB" CALLIGRAPHY_API_TOKENS="$TOKEN" \
    ./bin/calligraphy-api >"$LOGDIR/api.log" 2>&1 &
  api_pid=$!
...
start_workers() {
  ...
  for i in $(seq 1 "$n"); do
    env CALLIGRAPHY_DATABASE_URL="$DB" CALLIGRAPHY_WORKER_ID="bench-w$i" \
        CALLIGRAPHY_WORKER_METRICS_ADDR=":$((9100 + i))" CALLIGRAPHY_CONCURRENCY="$conc" "$@" \
        ./bin/calligraphy-worker >"$LOGDIR/worker-$i.log" 2>&1 &
    worker_pids+=($!)
  done
...
bench() { ./bin/calligraphy-bench -api "$API" -token "$TOKEN" "$@"; }
...
ablation)
  ...
  start_workers 1 1 CALLIGRAPHY_DB_MAX_CONNS=1 CALLIGRAPHY_REDIS_POOL_SIZE=1 \
    CALLIGRAPHY_BATCH_WRITES=false CALLIGRAPHY_FETCH_BATCH=1
  ...
  start_workers 1 1 CALLIGRAPHY_DB_MAX_CONNS=8 CALLIGRAPHY_REDIS_POOL_SIZE=8 \
    CALLIGRAPHY_BATCH_WRITES=false CALLIGRAPHY_FETCH_BATCH=1
  ...
  start_workers 1 8 CALLIGRAPHY_DB_MAX_CONNS=8 CALLIGRAPHY_REDIS_POOL_SIZE=8 \
    CALLIGRAPHY_BATCH_WRITES=false CALLIGRAPHY_FETCH_BATCH=8
  ...
  # v3: + batched writes
  start_workers 1 8 CALLIGRAPHY_DB_MAX_CONNS=8 CALLIGRAPHY_REDIS_POOL_SIZE=8 \
    CALLIGRAPHY_BATCH_WRITES=true CALLIGRAPHY_FETCH_BATCH=8
  ...
  stop_workers
  start_workers 1 4 CALLIGRAPHY_DB_MAX_CONNS=8 CALLIGRAPHY_REDIS_POOL_SIZE=8 \
    CALLIGRAPHY_BATCH_WRITES=true CALLIGRAPHY_FETCH_BATCH=8
...
reliability)
  start_api
  start_workers 7 4 CALLIGRAPHY_LEASE_TTL=10s
